...
const toPlainPreviewText = (value: any) => {
...
const withArticlePreview = (article: any) => {
  const source =
    toPlainPreviewText(article?.description) ||
    toPlainPreviewText(article?.excerpt) ||
    toPlainPreviewText(article?.contentMarkdown) ||
    toPlainPreviewText(article?.content);

  const description = source ? source.slice(0, 220) + (source.length > 220 ? "..." : "") : "";
  const excerpt = source ? source.slice(0, 140) + (source.length > 140 ? "..." : "") : "";

  return {
    ...article,
    description: article?.description ? (toPlainPreviewText(article.description) || description) : description,
    excerpt: article?.excerpt ? (toPlainPreviewText(article.excerpt) || excerpt) : excerpt,
  };
};
...
export const getAllMembers = async () => {
  ...
  try {
    ...
    return snapshot.docs.map((doc) => withArticlePreview({ id: doc.id, ...doc.data() }));
  } catch (error) {
...
export const getArticleById = async (articleId: string) => {
  ...
  try {
    ...
    if (docSnap.exists()) return withArticlePreview({ id: docSnap.id, ...docSnap.data() });
    return null;
  } catch (error) {
...
export const getArticlesByAuthor = async (authorId: string) => {
  ...
  try {
    ...
    // Fallback when composite index is not ready yet.
    if (code === "failed-precondition") {
      try {
        ...
        return fallbackSnap.docs
          .map((doc) => withArticlePreview({ id: doc.id, ...doc.data() }))
          .sort((a: any, b: any) => {
        ...
        console.error("Error fetching author articles fallback:", fallbackError);
        return [];
      }
    }
  ...
  try {
    ...
    const articles = await Promise.all(
      snapshot.docs.map(async (doc) => {
        ...
        return withArticlePreview({ id: doc.id, ...data, authorData });
      })
    );
    return articles;
  } catch (error) {
...
export const getConversation = async (userId1: string, userId2: string) => {
  if (!db) return [];
  try {
    // Avoid composite index dependency by querying per-sender and sorting client-side.
    ...
    const rows = [...sentByUser1.docs, ...sentByUser2.docs]
      .map((doc) => ({ id: doc.id, ...doc.data() }))
      .filter((row: any) =>
        (row.senderId === userId1 && row.receiverId === userId2) ||
        (row.senderId === userId2 && row.receiverId === userId1)
      )
      .sort((a: any, b: any) => {
...
export const markChatAsRead = async (userId: string, otherUserId: string) => {
  if (!db) throw new Error("Database not initialized");
  try {
    const chatRef = doc(db, "chats", userId, "chats", otherUserId);
    await setDoc(
      chatRef,
      {
        unread: false,
...
export const clearPendingMessagesFromSender = async (userId: string, senderId: string) => {
  if (!db) throw new Error("Database not initialized");
  try {
    const userRef = doc(db, "users", userId);
    const userSnap = await getDoc(userRef);
    const pendingIds: string[] = userSnap.data()?.pendingMessages || [];
    if (!pendingIds.length) return;

    const toRemove: string[] = [];
    await Promise.all(
      pendingIds.map(async (msgId) => {
        const msgSnap = await getDoc(doc(db, "messages", msgId));
        const data: any = msgSnap.data() || {};
        if (!msgSnap.exists()) {
          toRemove.push(msgId);
          return;
        }
        if (data.senderId === senderId || data.read === true) {
          toRemove.push(msgId);
        }
      })
    );
    ...
    if (toRemove.length > 0) {
      await updateDoc(userRef, { pendingMessages: arrayRemove(...toRemove) });
    }
  } catch (error) {
...
export const addReplyToComment = async (
  ...
  if (!db) throw new Error("Database not initialized");
  try {
    if (!postId || !commentId || !userId || !content?.trim()) {
